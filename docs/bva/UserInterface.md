...
For methods whose behaviour depends on user *input values* (e.g., `getNumberOfPlayers()`), we treat the textual console input as the variable under test and identify numeric and non‑numeric boundary conditions.  Display‑only methods (`displayWelcome`, `displayHelp`, etc.) do not produce functional *outputs* beyond printing, so their BVAs focus on the *state variations* that influence what is printed (e.g., empty vs. non‑empty hands).
...
|            | Input-1-(raw console string)                                                                                                            | Output-(return value or error print)                                                  |
|------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------|
| **Step-1** | text typed by the user                                                                                                                  | integer (2 – 5) *or* error message                                                    |
| **Step-2** | **Cases**                                                                                                                               | **Cases**                                                                             |
...
|             | System under test (sequence of inputs)         | Expected output / observations                                            | Implemented? | Test name                                              |
|-------------| ---------------------------------------------- | ------------------------------------------------------------------------- | ------------ | ------------------------------------------------------ |
...
## Method under test: `displayPlayerHand(Player player)`

The printed output varies with **hand size** and **card composition**.

### Step-1‑3-Results

|            | Input-1 (number of cards)   | Input-2-(card distribution)   | Output (printed hand listing)             |
|------------|-----------------------------|-------------------------------|-------------------------------------------|
| **Step-1** | integer `n` (0-–-hand-size) | multiset of `CardType` counts | formatted list, or "(empty)" when `n-=-0` |
...
|-------------| ------------------------------------------------------ | --------------------------------------------------------- | ------------ | ------------------------------------------- |
| Test-Case-1 | Hand size **0**                                        | prints "(empty)"                                          | ✅            | `displayPlayerHand_empty_showsEmptyMessage` |
| Test-Case-2 | Hand size **2**, both `SKIP`                           | prints indices **0** and **1**, each labelled `SKIP`      | ✅            | `displayPlayerHand_withCards_listsAll`      |
...
## Method under test: `displayError(String message)`

### Step-1‑3-Results

|            | Input-1-(message) | Output (stderr text)                    |
|------------|-------------------|-----------------------------------------|
| **Step-1** | arbitrary string  | "Error: "-+-`message` printed to stderr |
| **Step-2** | **Cases**         | **Printed result**                      |
| **Step-3** | non‑empty         | prepended with "Error: "                |
...
## Method under test: `getUserInput()`

### Step-1‑3-Results

|            | Input-1-(console line) | Output (returned string)    | Side‑effect (prompt) |
|------------|------------------------| --------------------------- | -------------------- |
| **Step-1** | arbitrary line         | same string without newline | prints "> " prompt   |
| **Step-2** | **Cases**              | **Returned**                | **Printed**          |
| **Step-3** | "hello world"          | "hello world"               | includes "> " prompt |
...
## Method under test: `displayCardPlayed(Card card)` & `displayDrawnCard(Card card)`

These methods share the same structure: they print a line referencing the card’s type.

### Step-1‑3-Results

|            | Input-1-(card type)    | Output (printed text)                    |
|------------|------------------------| ---------------------------------------- |
